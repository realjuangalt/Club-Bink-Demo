...
data_folder = "p2p_raw_data"
local_p2p_ads_buy = os.path.join(data_folder, "p2p_ads_buy.json")
local_p2p_ads_sell = os.path.join(data_folder, "p2p_ads_sell.json")
local_p2p_volume_buy = os.path.join(data_folder, "p2p_volume_buy.json")
local_p2p_volume_sell = os.path.join(data_folder, "p2p_volume_sell.json")

def get_p2p_market_data(currency):

    # if
        # Get p2p Data from API
        print("Get p2p Data from API")
    ...
    # except Exception as e:
     ...
     #online = p2p_volume_buy_response + p2p_volume_sell_response + p2p_ads_sell_response + p2p_ads_buy_response

    # if data_status == 'Offline':
        print(f"Data Status: {data_status}, transitioning to local data.")
        pass
        ...
        ads_count = local_p2p_volume_buy['stats']['ads_count']

        sorted_exchanges = sorted(ads_count.items(), key=lambda x: x[1], reverse=True) - blacklist
        top_3_exchanges_buy = [exchange[0] for exchange in sorted_exchanges[:3]]

        buy_whitelist = set(top_3_exchanges_buy)
        # print("Top 3 buy side exchanges:", buy_whitelist)

        if online is True:  
            ads_count = p2p_volume_sell_response['stats']['ads_count']
        else:
            ads_count = local_p2p_volume_sell['stats']['ads_count']
